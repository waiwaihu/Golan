...
- tar -zxf mongodb-linux-x86_64-rhel70-5.0.16.tgz
```
...
```markdown
# 3.查看安装目录
- cd mongodb-linux-x86_64-rhel70-5.0.16
- ls
...
```markdown
# 2.运行 mongo 镜像
- docker run -d --name mongo -p 27017:27017 mongo:5.0.16

$ docker run -d --name mongo -p 27017:27017 \
	-e MONGO_INITDB_ROOT_USERNAME=mongoadmin \
	-e MONGO_INITDB_ROOT_PASSWORD=mongo12345 \
	mongo:5.0.16
...
- 单条文档
  ...
  ```sql
  db.comment.insert({"_id":1,"articleid":"1","content":"今天天气真好，阳光明媚","userid":"1001","nickname":"Rose","createdatetime":new Date(),"likenum":NumberInt(10),"state":null})
  ```
...
{"_id":"5","articleid":"100001","content":"研究表明，刚烧开的水千万不能喝，因为烫嘴。","userid":"1003","nickname":"凯撒","createdatetime":new Date("2019-08-
...
```sql
db.集合名称.find({userid:'1001'})
```

> 如果你只需要返回符合条件的第一条数据，我们可以使用findOne命令来实现，语法和find一样。

```sql
db.集合名称.findOne({userid:'1001'})
```

#### 投影查询

> 如果要查询结果返回部分字段，则需要使用投影查询（不显示所有字段，只显示指定的字段）。 如：查询结果只显示 _id、articleid、nickname :

```sql
> db.comment.find({userid:"1001"},{articleid:"1",nickname:"Rose"})
```
...
db.users.find({age:23,name:"编程不良人_1"}).pretty()
db.users.find({age:($gt:23),name:"编程不良人_1"}).pretty()
```
...
> db.users.find({$or:[{name:"编程不良人_20"} ,{name:"编程不良人_10"}]})
> db.users.find({$or:[{name:"编程不良人_20"} ,{name:"编程不良人_10"}]}).pretty()
```
...
`类似 SQL 语句为: 'order by name AES,age DESC'`
...
db.users.find({name:{$in:["编程不良人_20","编程不良人_19"]}})
